...
def plot_author_pair(df, value = 'HC', wrt_authors = [],
    ...
    p = (
        ggplot(data = df1, mapping = aes(x='x', y='y', color='author', shape = 'author')) +
...
def plot_LDA(df, value, wrt_authors, sym = False) : 
    ...
    p = (
        ggplot(data=df_plot, mapping = aes(
                x='t',
                y='value',
                color='author',
                fill='author',
...
def plot_col(df, value, sign, wrt_authors = []) :
    ...
    p = (ggplot(data=df.dropna(), mapping = aes(x='term', y=value, fill='used more in')) +
